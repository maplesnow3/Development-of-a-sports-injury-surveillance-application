				...
				<Button  // "Check selected results" button
					...
					onClick={() => {
						if (!dateRangeSelected || dateRangeSelected.length < 2) {
							...
							window.location.href = `/front/#/record_browser/list?uid=1&date_from=${dateRangeSelected[0].format("YYYY-MM-DD")}&date_to=${dateRangeSelected[1].format("YYYY-MM-DD")}`
						}
					} }
				>Show</Button>
			</div>
				...
				<Calendar  // Report calendar
					...
					onSelect={ (dateSelected) => {
						...
						if (datesWithRecord.indexOf(dateString) > -1) {
							...
							window.location.href = `/front/#/record_browser/list?uid=self&date_from=${dateString}&date_to=${dateString}`
						}
					} }
				/>
			</div>
